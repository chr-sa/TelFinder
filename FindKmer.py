...
def chrKmerFind(seq1, kmer_start, kmer_end):
    ...
    for kmer_size in range(kmer_start, kmer_end):
        ...
        component_dc, gap_dis_dc, gap_base_dc = kmer_info(
            kmer_size,
            kmer_site,
            kmer_repeat,
            component_dc,
...
#########################
def judge(chr1, seq1, kmer_start, kmer_end, outf):
    count1 = collections.Counter(list(seq1))
    if count1["C"] / len(seq1) >= 0.4 or count1["G"] / len("G") / len(seq1) >= 0.4:
