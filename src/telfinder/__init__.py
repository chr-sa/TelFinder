from telfinder.FindKmer import *  # kmer find
from telfinder.statMut import *  # statistic about kmers
from telfinder.score import *  # score
...
def main():
    ...
    parser.add_argument(
        ...
        "--format",
        required=True,
        help="format of input genome. fa means fasta, fq1 means single end fastq, fq2 means paired end fastq (for identifying telomere unit); kmer means kmer candidates(for mutation analysis)",
    )
    parser.add_argument(
        "-inf", "--input-file", help="input file (fa or fq(single end) genome file or kmer candidates file)"
    )
    ...
    parser.add_argument(
        ...
        "--seq-len",
        default="1000",
        help="needed when -f fa is used, search region (nt) from chromosome termini. (Default: = 1000)",
    )
            ...
            for chr1 in chrEndSeq:
                ...
                for kmer in component_dc:
                    ...
                    out1.write(
                        ...
                        + str(kmer)
                        + "\t"
                        + str(component_dc[kmer][0])
                        + "\t"
                        + str(component_dc[kmer][1])
                        + "\t"
                        + gap_dis_info
                        + "\t"
                        + gap_base_info
        ...
        if "fq" in input_format:
            ...
            if input_format == "fq1":
                ...
                os.system(
                    ...
                    + tnumber
                    + " -x "
                    + os.path.join(out_dir, species)
                    + " -U "
                    + input_file
                    + " -S "
                    + os.path.join(out_dir, species + ".sam")
                )
